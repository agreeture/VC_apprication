...
class iSTFTNET_FC_for_Generator(torch.nn.Module):
    """iSTFTNET generator module."""
    ...
    def __init__(
        ...
        for i in range(len(upsample_scales)):
            self.up_rate = self.up_rate * upsample_scales[i]
        self.hop_size = 256
        self.fft_size = 1024
        self.f_s = self.fft_size // self.up_rate
        self.h_s = self.fft_size // self.hop_size
        ...
        if not use_causal_conv:
            self.output_conv_1 = torch.nn.Sequential(
            ...
            self.output_conv_2 = torch.nn.Sequential(
                # NOTE(kan-bayashi): follow official implementation but why
                #   using different slope parameter here? (0.1 vs. 0.01)
                torch.nn.LeakyReLU(),
                torch.nn.Conv1d(
        ...
        self.dense = torch.nn.Linear(9 * 2, self.f_s)
    ...
    def forward(self, c):
        ...
        c_1 = self.output_conv_1(c)
        c_2 = self.output_conv_2(c)
        c = torch.transpose(torch.cat((c_1, c_2), 1), 1, 2)
        c = self.dense(c)
        c_stack = torch.zeros([B, self.hop_size * T + self.f_s - self.h_s]).to('cuda:0')
        for i in range(self.up_rate * T):
            start_pos = i * self.h_s
            end_pos = i * self.h_s + self.f_s
            c_stack[:, start_pos:end_pos] = c_stack[:, start_pos:end_pos] + c[:, i, :]
        c = c_stack[:, :self.hop_size * T]
        c = torch.reshape(c, (B, 1, c.shape[1]))
