...
class MS_iSTFT_HiFiGAN_Generator(torch.nn.Module):
    ...
    def __init__(
        ...
        channels=512,
        kernel_size=7,
        filter_kernel_size=63,
        simple_upsample_scale_fc=4,
        simple_upsample_scale_ms=4,
        upsample_scales=(8, 8),
        ...
        for i in range(len(upsample_kernel_sizes)):
            ...
            self.upsamples += [
                torch.nn.Sequential(
                    ...
                    #torch.nn.Conv1d(channels//(2**i), channels//(2**(i+1))*upsample_scales[i],
                    #                upsample_kernel_sizes[i]-1, 1, padding=(upsample_kernel_sizes[i]-1)//2),
                    #PixelShuffle1D(upsample_scales[i]),
                )
            ]
        ...
        self.output_conv = torch.nn.Sequential(
            ...
            torch.nn.Conv1d(
                channels // (2 ** (i + 1)),
                ((simple_upsample_scale_fc * 2 + 1) * 2) * simple_upsample_scale_ms,
                kernel_size,
                1,
                padding=(kernel_size - 1) // 2,
            ),
        ...
        self.subbands = simple_upsample_scale_ms
        self.fc_rate = simple_upsample_scale_fc
        self.synthesis_filter = torch.nn.Conv1d(simple_upsample_scale_ms, 1, filter_kernel_size, 1, bias=False)
        self.pad_fn = torch.nn.ConstantPad1d( (filter_kernel_size-1) // 2, 0.0)

        #self.padding = torch.nn.ZeroPad2d((0, 0, 0, 1, 0, 0, 0, 0))
    ...
    def forward(self, c):
        ...
        c_out = c.shape[1]
        l = c.shape[2]
        c = torch.reshape(c, (c.shape[0], self.subbands, (self.fc_rate * 2 + 1)*2, l))

        spec = torch.exp(c[:,:,:(self.fc_rate * 2 + 1), :])
        phase = math.pi*torch.sin(c[:,:, (self.fc_rate * 2 + 1):, :])
        spec = torch.reshape(spec, (spec.shape[0]*self.subbands, self.fc_rate * 2 + 1, spec.shape[-1]))
        phase = torch.reshape(phase, (phase.shape[0]*self.subbands, self.fc_rate * 2 + 1, phase.shape[-1]))
        
        c = torch.istft(spec*torch.exp(phase*1j), n_fft=self.fc_rate*4, hop_length=self.fc_rate, win_length=self.fc_rate*4)
        c = torch.reshape(c, (c.shape[0] // self.subbands, self.subbands, 1, c.shape[-1]))
        c = c.squeeze(-2)
        
        updown_filter = torch.zeros((self.subbands, self.subbands, self.subbands)).float()
        #updown_filter = torch.zeros((self.subbands, self.subbands, self.subbands)).float().to("cuda")
        for k in range(self.subbands):
            updown_filter[k, k, 0] = 1.0
        c = F.conv_transpose1d(c, updown_filter * self.subbands, stride=self.subbands)
        c = self.synthesis_filter(self.pad_fn(c))
        return c
