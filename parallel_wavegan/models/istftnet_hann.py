...
class iSTFTNET_hann_Generator(torch.nn.Module):
    ...
    def forward(self, c):
        ...
        c = torch.stack([c_1, c_2], dim=-1)
        c = self.padding(c)
        c = torch.istft(c, n_fft=16, hop_length=4, win_length=16, window=torch.hann_window(window_length=16).to("cuda:0"))
        c = torch.reshape(c, (batch_size, 1, c.shape[1]))
