...
class iSTFTNET_FC_noforloop_Generator(torch.nn.Module):
    ...
    def forward(self, c):
        ...
        c = self.dense(c)
        c = self.sum_hop(c, self.h_s)

        return c

    def sum_hop(self, x, hop_size):
        b = x.shape[0]
        t = x.shape[1]
        c = x.shape[2]
        ...
        x = torch.nn.ZeroPad2d((0, pad_size, 0, 0, 0, 0))(x)
        x = x.view(b, 1, -1)
        x = x[:, :, :-pad_size]
        x = x.view(b, t, c+pad_size-hop_size)
        x = torch.sum(x, 1)
        x = x.view(b, 1, -1)
        x = x[:, :, 6:-6]
